...
**日期：** 2026-07-23（v0.5 点选飞入）  
**状态：** POC 实机定稿（对齐 `phys.ts` + `14` **v1.3**）  
**事件：** `10_physical_ue_events.md`  
**实现钉：** `14_physical_impl_pins.md` **v1.3**  
...
| 路径 | 流程 | 定稿 |
|------|------|------|
| **点消** | A1 **直线→A2**（A2 不动）→ 同拖 exit | ✅ v1.3 |
| **拖消** | **skipMeet** → exit（松手 + throwForceK + approachDir） | ✅ |
| 抛角 | ∝ 飞入 / 拖速方向（bias 0.34） | ✅ |
| 飞入弧 | **关** | ✅ |
| exit 结束 | **出屏** 或 hard | ✅ |
| 淡出 | **关** | ✅ |
| pivot | **默认牌心** | ✅ |
